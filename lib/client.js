      ...
      const insert = () => {
        ...
        try {
          ...
          if (inputTriggers && typeof input.insertReference === 'function') {
            ...
            // 注意：chip 在 composer 里只占 1 个字符宽度（backdrop 绝对定位 + scale(.72)），
            // 多字符 label（如"引用#1"）会溢出格子盖住旁边的输入文字；
            // 因此显示 label 用 1 字符的引用符号，完整编号保留在悬停提示（refTargets.label）里。
            const span = { start: st.draft.length, end: st.draft.length, draftRev: st.draftRev }
            const applied = input.insertReference({ source: 'quote-ref', ref, label: '❝', clipboardText: quoteBody }, span)
            if (applied) {
              // 把评论追加到 chip 之后
              if (commentTail !== '') {
